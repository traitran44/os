...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
    ...
    void * fault_page = (void*)(fault_addr - (fault_addr % page_size));
    if ((uintptr_t)prev_pg != 0) {
        if (munmap(prev_pg, page_size) == -1) {
            fprintf(stderr, "Couldn't munmap() region for sqrt table; %s\n",
                    strerror(errno));
            exit(EXIT_FAILURE);
        }
    }
    void *new_Pg = mmap((void *)fault_page,page_size, PROT_READ|PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (new_Pg == MAP_FAILED) {
        printf("Failed to map new page\n");
    } else {
        prev_pg = new_Pg;
    }
    calculate_sqrts((double *)new_Pg, ((double *)new_Pg) - sqrts, page_size / sizeof(double));
}
